...
ExtraContainerChanges::EntryDataList* TESObjectREFR::GetContainerChangesList() const {
...
{
	auto const from = this;
	float result = (atan2(to->posX - from->posX, to->posY - from->posY) - from->rotZ) * 57.29578f;
	if (result < -180)
		result += 360;
	else if (result > 180)
		result -= 360;
...
{
	float opposite = out->posZ - to->posZ;
	float hypotenuse = out->GetDistance(to);
	float fraction = opposite / hypotenuse;
	return (asin(fraction) - out->rotX) * Flt180dPI;
}
